...
const ManageBooks= () => {
    ...
    useEffect(() => {
        fetch("https://bookstore-project-essg.onrender.com/api/books").then(res => res.json()).then(data => setAllBooks(data));
    }, [])
    //delete a book
            ...
            </Table.Head>
            {
                allBooks.map((book, index) => <Table.Body className='divide-y' key={book.id}>
                    <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <Table.Cell>
                            <Link className='font-medium text-cyan-600 hover:underline dark:text-cyan-500 mr-5' to={`/admin/dashboard/edit-allBooks/${book.id}`}>
